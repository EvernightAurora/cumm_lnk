...
def grouped_conv_gemm_ref(a, b, m, n, k, g, conv_op):
    if conv_op == ConvOpType.kForward:
        c = np.zeros([m, n*g], dtype=a.dtype)
        for i in range(g):
            c[:, (i*n):((i+1)*n)] = (a[:, (i*k) : ((i+1)*k)] @ b[:, (i*n):((i+1)*n)])
    elif conv_op == ConvOpType.kBackwardInput:
        c = np.zeros([m, n*g], dtype=a.dtype)
        for i in range(g):
            c[:, (i*n):((i+1)*n)] = (a[:, (i*k) : ((i+1)*k)] @ b[(i*k):((i+1)*k), :])
    else:
        c = np.zeros([m*g, n], dtype=a.dtype)
        for i in range(g):
            c[(i*m):((i+1)*m), :] = (a[(i*m):((i+1)*m), :] @ b[:, (i*n):((i+1)*n)])
    return c
...
def _asdv_test_turing_python(coord_input: bool = False):
    ...
    with cudasim.enter_debug_context(True, 3):
        ...
        for params in main_cu.all_params[:]:
            ...
            # n = max(params.ts[1], n)
            # k = max(params.ts[2], k)
            if ker.conv_type != ConvOpType.kBackwardWeight:
                m = max(m, 48)
            else:
                k = max(k, 48)
            if ker.conv_type == ConvOpType.kForward:
                a_shape = [m, k*g]
                b_shape = [k, g*n]
                c_shape = [m, g*n]
            elif ker.conv_type == ConvOpType.kBackwardInput:
                a_shape = [m, k*g]
                b_shape = [k*g, n]
                c_shape = [m, n*g]
            else:
                ...
                b_shape = [k, g*n]
                c_shape = [g*m, n]
            print(a_shape,  b_shape,  c_shape,  ker.conv_type)
            if params.dtype_a == dtypes.int8:
                a = np.random.randint(-2, 2, size=[m, k]).astype(np.int8)
                b = np.random.randint(-2, 2, size=[k, n]).astype(np.int8)
                dtype_c = params.dtype_c.npdtype()
                c = -grouped_conv_gemm_ref(a.astype(np.float32), b.astype(np.float32), m, n, k, g, ker.conv_type).astype(np.int8)

            else:
                a = np.random.uniform(-1, 1, size=a_shape).astype(
                    dtypes.get_npdtype(params.dtype_a))
                b = np.random.uniform(-1, 1, size=b_shape).astype(
                    dtypes.get_npdtype(params.dtype_b))
                # a[:, 32:] = 0
                # b[32:] = 0

                c = -grouped_conv_gemm_ref(a, b, m, n, k, g, ker.conv_type)
                # c += np.arange(np.prod(c.shape)).reshape(c.shape)
            if params.trans_a:
                a = np.ascontiguousarray(a.transpose(1, 0))
            if params.trans_b:
                b = np.ascontiguousarray(b.transpose(1, 0))
            ...
            if coord_input:
                if params.trans_a:
                    for i in range(a_meta.shape[0]):
                        for j in range(a_meta.shape[1]):
                            a_meta[i, j] = (i * a_meta.shape[1] + j)
                else:
                    for i in range(a_meta.shape[0]):
                        for j in range(a_meta.shape[1]):
                            a_meta[i, j] = (i * a_meta.shape[1] + j)
                if params.trans_b:
                    for i in range(b_meta.shape[0]):
                        for j in range(b_meta.shape[1]):
                            b_meta[i, j] = (i * b_meta.shape[1] + j)
                else:
                    for i in range(b_meta.shape[0]):
                        for j in range(b_meta.shape[1]):
                            b_meta[i, j] = (i * b_meta.shape[1] + j)
